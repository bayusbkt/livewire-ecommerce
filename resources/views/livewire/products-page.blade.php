            ...
            <div class="flex flex-wrap mb-24 -mx-3">
                <div class="w-full pr-2 lg:w-1/4 lg:block">
                                ...
                                <li class="mb-4" wire:key="{{ $category->id }}">
                                    <label for="{{ $category->slug }}" class="flex items-center dark:text-gray-400 ">
                                        <input type="checkbox" id="{{ $category->slug }}" value="{{ $category->id }}"
                                            class="w-4 h-4 mr-2" wire:model.live="selected_categories">
                                ...
                                <li class="mb-4" wire:key="{{ $brand->id }}">
                                    <label for="{{ $brand->slug }}" class="flex items-center dark:text-gray-300">
                                        <input type="checkbox" id="{{ $brand->slug }}" value="{{ $brand->id }}"
                                            class="w-4 h-4 mr-2" wire:model.live="selected_brands">
                    ...
                    <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
                        ...
                        <div class="w-16 pb-2 mb-6 border-b border-rose-600 dark:border-gray-400"></div>
                        <div>
                            <div class="font-semibold dark:text-white">{{Number::currency($range, 'IDR')}}</div>
                            <input type="range"
                                class="w-full h-1 mb-4 bg-blue-100 rounded appearance-none cursor-pointer"
                                max="50000000" value="25000000" step="10000" wire:model.live="range">
                            <div class="flex justify-between ">
                                <span class="inline-block text-lg font-bold text-blue-400 ">10,000.00</span>
                                <span class="inline-block text-lg font-bold text-blue-400 ">50,000,000.00</span>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="w-full px-3 lg:w-3/4">
                    ...
                    @if ($products->isEmpty())
                        <div class="w-full text-center py-10">
                            <h2 class="text-2xl font-semibold text-gray-500 dark:text-gray-400">No Products Available
                            </h2>
                        </div>
                    @else
